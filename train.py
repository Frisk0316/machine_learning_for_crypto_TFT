"""
train.py — v5: Cross-sectional ranking training for LSTM / TFT.

Key changes from v4:
  1. ListNet ranking loss (replaces MSE) — directly optimises cross-sectional ranking
  2. Cross-sectional batch training — all assets at same time step per forward pass
  3. Linear warmup + ReduceLROnPlateau — stabilise early training
  4. Stronger regularisation (dropout=0.30, weight_decay=1e-4)

Usage:
    python train.py --config config.json [--model tft|lstm]
"""
...
def listnet_loss(pred, target, mask, temperature=1.0):
    ...
    For each time step, we compare the predicted ranking (softmax of scores)
    against the ideal ranking (softmax of actual returns). This directly
    optimises the model to produce correct relative orderings.
    ...
    p_true = F.softmax(target_valid / temperature, dim=0)
...
def train_one_seed(cfg, data, train_idx, valid_idx, test_idx,
    ...
    for epoch in range(cfg['epochs']):
        ...
        # ── Train: iterate over time steps in random order ──
